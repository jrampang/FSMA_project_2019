...
public class PreneurAutoBehaviour extends Behaviour {
	...
	public void action() {
		...
		if(msgReceived != null) {
			...
			System.out.println("Auto behaviour: msgReceived from " + name);
			if(msgReceived.getPerformative() == ACLMessage.INFORM) {
				System.out.println("Auto behaviour: it's a rep_bid.");
				String content = msgReceived.getContent();
				System.out.println("Auto behaviour: content " + content);
			}
			if(msgReceived.getPerformative() == ACLMessage.ACCEPT_PROPOSAL) {
				System.out.println("Auto behaviour: it's a to_attribute.");
				to_pay.addReceiver(new AID(name, AID.ISLOCALNAME));
				owner.send(to_pay);
			}
			if(msgReceived.getPerformative() == ACLMessage.AGREE) {
				System.out.println("Auto behaviour: it's a tp_give.");
				System.out.println("Auto behaviour: i received my fish.");
			}
		}
		else {
			block();
